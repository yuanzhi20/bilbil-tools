...
// JSON
function formatJSON() {
  try { document.getElementById('jsonOutput').textContent = JSON.stringify(JSON.parse(document.getElementById('jsonInput').value), null, 2) }
  catch (e) { document.getElementById('jsonOutput').textContent = '❌ ' + e.message }
}
function compressJSON() {
  try { document.getElementById('jsonOutput').textContent = JSON.stringify(JSON.parse(document.getElementById('jsonInput').value)) }
  catch (e) { document.getElementById('jsonOutput').textContent = '❌ ' + e.message }
}
...
function tsToDate() {
  const ts = document.getElementById('ts2date').value;
  const s = ts.length > 10 ? parseInt(ts) : parseInt(ts) * 1000;
  document.getElementById('ts2dateResult').textContent = new Date(s).toLocaleString('zh-CN');
}
function dateToTs() {
  const d = new Date(document.getElementById('date2ts').value);
  document.getElementById('date2tsResult').textContent = Math.floor(d.getTime() / 1000);
}

// Hash
async function calcHash() {
  const data = new TextEncoder().encode(document.getElementById('hashInput').value);
  const hash = await crypto.subtle.digest(document.getElementById('hashAlgo').value.replace('-', ''), data);
...
function setupColor() {
  ...
  p.addEventListener('input', e => { document.getElementById('hexInput').value = e.target.value; updateColor(e.target.value) });
  document.getElementById('hexInput').addEventListener('input', e => { if (/^#[0-9a-f]{6}$/i.test(e.target.value)) { p.value = e.target.value; updateColor(e.target.value) } });
}
function updateColor(hex) {
  ...
  document.getElementById('colorAll').textContent = `HEX: ${hex} | RGB: rgb(${r},${g},${b}) | HSL: hsl(${Math.round(h)},${Math.round(s)}%,${Math.round(l)}%)`;
}
...
function generateQR() {
  ...
  // 用Google Charts API生成（无需额外库）
  const url = `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encodeURIComponent(text)}`;
  const container = document.getElementById('qrResult');
  container.innerHTML = `<img src="${url}" alt="QR Code" style="border:1px solid #ddd;border-radius:8px"><br>
    <a href="${url}" download="qrcode.png" style="display:inline-block;margin-top:.5rem;padding:.5rem 1rem;background:#667eea;color:#fff;text-decoration:none;border-radius:6px">下载二维码</a>`;
}

// Image compress
function compressImg() {
  const files = document.getElementById('imgInput').files;
  const q = document.getElementById('imgQuality').value / 100;
  ...
  for (const f of files) {
    ...
    reader.onload = e => {
      ...
      img.onload = () => {
        ...
        a.textContent = `${f.name}: ${(f.size / 1024).toFixed(1)}KB → ${(a.href.length * 3 / 4 / 1024).toFixed(1)}KB`;
        a.style.cssText = 'display:block;margin:.5rem 0;color:#667eea';
